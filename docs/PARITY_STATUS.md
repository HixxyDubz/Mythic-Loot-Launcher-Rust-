...
| Safe Launch | Not implemented | Optional-mod disabling, persistent crash recovery and restore-after-game-exit orchestration remain to be ported. No server behavior will be added. |
...
- `cargo test --manifest-path src-tauri/Cargo.toml`: 34 passed, including schema/config recovery, bundled manifests, adversarial paths, streaming hashes, deterministic publishing, privacy scanning, fail-closed GitHub actions, changed-file repair, update rollback, restore staging, update-created path removal, unsafe/legacy backup rejection, five-point retention and restore rollback.
...
- Release executable: 16,524,800 bytes; SHA-256 `9BD7167FB7B46ABB73047111AE1664CA0B231B4E7567E89D0A23DB005DEED441`.
- Isolated-data native smoke: responsive at the four-second sample, 30.0 MB working set, schema 2, two profiles, and no `serverName`, `serverIp` or `serverPort` keys; temporary data was removed afterward.
- Browser visual check: the Update & Repair and Recovery history workspace passed at 1180x760 and 940x640 without horizontal overflow; its safety sequence, preparation actions and recovery empty state remained readable at both sizes.

## External acceptance boundary

Server acceptance is not a launcher gate because servers are managed elsewhere. A full real-package GitHub update/repair, real large-installation restore, authenticated GitHub repository creation/release publication, multipart publication, separate Player/Developer artifacts, Safe Launch and friend-machine modpack acceptance are not part of this checkpoint.
