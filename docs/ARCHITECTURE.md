...
GitHub publishing is a separate Developer workflow: local preparation scans privacy and produces reviewed artifacts without authentication; repository creation, release publication and public-catalogue replacement use authenticated `gh` state and separate explicit confirmations. The Developer content editor is local-only: it accepts a profile id and typed presentation fields, clones the existing trusted manifest (or creates a real profile-bound draft before the first release), changes no package metadata, validates the complete candidate and atomically saves it. Packages below 2 GiB remain one deterministic ZIP. Larger packages are split byte-for-byte into ordered 1 GiB assets, with each part and the complete ZIP represented by SHA-256 in the trusted manifest. The updater verifies each download, reconstructs the original ZIP, verifies its complete hash, then enters the existing stage/verify transaction. Native release and catalogue commands accept only cached preview identifiers, not arbitrary frontend paths, and re-hash every reviewed artifact at action time. Catalogue publication targets only the fixed launcher repository, branch and filename through GitHub's Contents API; it never accepts a token or arbitrary destination from React. Player builds compile without the `developer` Cargo feature, so `publisher.rs`, `packager.rs`, `catalog_publisher.rs`, `content_editor.rs`, their seven Tauri command registrations, and the frontend Publisher route are absent rather than hidden by a runtime toggle.
