...
- `app_update_publisher.rs` (Developer feature only): build-manifest validation, independent Player artifact re-hashing, fixed checksum-feed generation, exact release-plan caching and explicitly confirmed immutable launcher release publication.
