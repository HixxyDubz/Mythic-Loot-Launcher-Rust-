...
This repository is the maintained Rust/TypeScript application. **MLLP** (Mythic Loot Launcher Python) is retired and reference-only: do not upgrade it, package it, or copy these fixes into it. It is not a runtime dependency.

## Current status

Version 0.1.12 adds read-only Minecraft version/loader detection, reviewed Publisher draft filling and advisory comparison with published requirements in Settings. See [the metadata checkpoint](docs/MINECRAFT_METADATA_0.1.12.md), [preferences/runtime tools](docs/PREFERENCES_RUNTIME_0.1.11.md), [optional mods](docs/OPTIONAL_EXTRAS_0.1.10.md), [release setup](docs/RELEASE_SETUP_0.1.9.md), and [blocker fixes](docs/BLOCKER_FIXES_0.1.8.md). This is not a claim of complete Python parity or zero bugs; remaining work is tracked in [the completion checklist](docs/FEATURE_COMPLETION.md). The 7DTD public manifest still needs a real published package; a built installer is not itself a new GitHub app release.

The application currently has a working native shell, server-free modpack profile store, Developer-only modpack creation, built-in game catalogue, local installation detection, trusted v1 manifest validation, streaming SHA-256 file verification, readiness assessment, profile editing, native game launch, transactional modpack update/repair and restore-point recovery, Smart Launch orchestration, persistent Safe Launch recovery, a Player application updater, and local-first GitHub publishing workflows. Smart Launch checks every tracked file, chooses version update or changed-file repair through the same native transaction engine, requires explicit approval before live mutation, rechecks the complete manifest, and opens the game or selected Minecraft launcher only after success. Updates and repairs download or accept a trusted package, validate and stage it away from the live installation, preview the exact mutation, create a pre-change backup, apply only after explicit confirmation, post-verify every required file, and roll back on failure. Recovery history retains the newest five metadata-backed points; restoring one is independently staged, previewed, confirmed, protected by a second backup, verified and rollback-capable. Safe Launch moves only trusted-manifest optional files, records their exact hashes outside the installation, waits for the launched game process, and restores them after exit or through a guarded restart recovery. Player app updates use a fixed repository/feed, reviewed size and SHA-256, launcher-owned staging and a verified external replacement/rollback helper. Developer can prepare exactly the Player portable, installer and checksum feed from a real Windows build manifest, then publish only after a second explicit confirmation. The Developer Publisher also privacy-scans modpack sources, builds deterministic packages/manifests and exposes separately reviewed GitHub release and public-catalogue actions. Player is compiled without Developer publication UI, metadata editing, or native publication commands. Production has no browser-preview profiles or fallback launcher state: every API path requires the native Rust application and fails closed otherwise. The complete parity position is tracked in [docs/PARITY_STATUS.md](docs/PARITY_STATUS.md).

Player startup renders from bundled and last-verified local data immediately. With startup update checks enabled (the default), it then refreshes the public [`launcher-catalog.json`](launcher-catalog.json) from the launcher's public `main` branch and each modpack's dedicated manifest asynchronously from GitHub. Turning off startup checks leaves explicit **Refresh catalogue now** and **App update** actions available. A late startup response never replaces a settings draft or newer visible state. The catalogue never contains machine-local paths or launcher/account state. Remote metadata is size-bounded and schema/path/identity validated before atomic cache replacement; offline or invalid responses leave the previous verified data active. Developer treats its local profile metadata as authoritative: new packs begin as hidden drafts, a successful release enables catalogue visibility, and unchecking visibility archives a pack from the next reviewed catalogue publication.
...
The Developer edition's **App update** workspace reads that build manifest, re-hashes the packaged Player artifacts and stages stable release names plus `launcher-update-player.json` for review. Keep `package.json`, `src-tauri/Cargo.toml` and `src-tauri/tauri.conf.json` on the same semantic version before rebuilding. Publishing is fixed to `HixxyDubz/Mythic-Loot-Launcher-Rust-`, requires authenticated GitHub CLI state and explicit confirmation, and never includes the Developer executable. `scripts/Publish-LiveAppRelease.ps1` drives that visible workflow only when `-ConfirmPublish` is supplied, then verifies the exact three-asset latest release. `npm run smoke:live-app-update -- -FromVersion <old> -ExpectedVersion <new>` performs an isolated live transition from a preserved released Player. Historical 0.1.6-to-0.1.7 acceptance used the spaced installed filename, not the hyphenated GitHub filename; the latter exposed a helper bug fixed in 0.1.8. An already-downloaded 0.1.7 portable with the hyphenated name still contains its old helper: close it and rename it to `Mythic Loot Launcher Player.exe` for a supported one-time upgrade path once a newer release is published. New 0.1.8 builds accept all three shipped Player filenames and wait for helper readiness before closing.
