...
The Developer edition's **App update** workspace reads that build manifest, re-hashes the packaged Player artifacts and stages stable release names plus `launcher-update-player.json` for review. Keep `src-tauri/Cargo.toml` and `src-tauri/tauri.conf.json` on the same semantic version before rebuilding. Publishing is fixed to `HixxyDubz/Mythic-Loot-Launcher-Rust-`, requires authenticated GitHub CLI state and explicit confirmation, and never includes the Developer executable. The first real release is intentionally not automated by tests or packaging.
