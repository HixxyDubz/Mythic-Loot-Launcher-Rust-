...
Filesystem mutation, game detection, process launch, updates, backups, and publishing belong in Rust. React is an untrusted presentation layer and only receives narrow Tauri commands. Publishing scans relative paths and text content without treating an incidental source-root username as a leak, excludes known runtime/private state, rejects credential-shaped data, and re-hashes cached native assets immediately before upload. Update sources must remain dedicated HTTPS/package locations; Discord invites are never downloads. The updater retains MLLP's stage, verify, backup, apply, post-verify, and rollback contract, caches native previews instead of accepting frontend paths at apply time, and refuses to stage inside the live modpack.
