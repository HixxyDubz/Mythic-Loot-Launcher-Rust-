...
For Minecraft, CurseForge and Modrinth instances are supported sync targets. Create or import the matching Minecraft/NeoForge profile in the chosen launcher, select it through **Detect installs**, then use **Sync, update & repair**. The existing staged transaction applies only trusted release inventory; it does not synchronize saves, logs, screenshots, options, caches or launcher/account metadata. Current and legacy Modrinth profile locations are detected. Automatic launcher-native creation/import of a brand-new profile remains pending.
