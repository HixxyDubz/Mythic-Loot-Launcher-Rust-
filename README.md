...
Version 0.1.8 addresses the five high-priority findings from the adversarial review: Player update filename/readiness handling, large-download timeout/resume handling, overlapping maintenance and settings writes, stale profile approvals, and interrupted configuration saves. See [the blocker-fix checkpoint](docs/BLOCKER_FIXES_0.1.8.md) for verification and remaining release gates. This is not a claim of complete Python parity or zero bugs. The 7DTD public manifest still needs a real published package; a built installer is not itself a new GitHub app release.
